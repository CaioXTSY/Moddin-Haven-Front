    ...
    return (
      <section className="relative w-full">
            <div className="mx-auto max-w-7xl px-6 py-24">
              <div className="border border-zinc-700 bg-zinc-900 p-8 text-center rounded-xl shadow-md shadow-purple-900/20">
                ...
                <div className="mt-4">
                  <Link href="/mods" className="border border-mauve-700 px-3 py-1.5 text-sm text-mauve-600 hover:bg-mauve-600 hover:text-black rounded-md transition">Back to mods →</Link>
                </div>
              </div>
            </div>
      </section>
      ...
      <div className="mx-auto max-w-7xl px-6 py-16">
        ...
        <div className="mt-4 grid grid-cols-1 gap-6 md:grid-cols-3">
          <div className="md:col-span-2 space-y-6">
            <div className="border border-zinc-700 bg-zinc-900 overflow-hidden rounded-xl max-w-[680px] transition-all hover:border-mauve-700 hover:shadow-md hover:shadow-purple-900/20">
            ...
            <div>
              ...
              <div className="mt-3 grid grid-cols-3 gap-3 max-w-[680px]">
                {(mod.gallerySrcs ?? [mod.imageSrc]).slice(0, 6).map((src, i) => (
                  <div key={`${src}-${i}`} className="group relative w-full overflow-hidden rounded-lg border border-zinc-700 cursor-pointer transition-all hover:-translate-y-0.5 hover:border-mauve-700 hover:shadow-md hover:shadow-purple-900/20">
                    <div className="absolute inset-0 opacity-0 transition-opacity group-hover:opacity-10 bg-mauve-600" />
            ...
            <div>
              ...
              <div className="mt-3 border border-zinc-700 bg-zinc-900 p-4 text-sm text-zinc-300 rounded-md transition-all hover:border-mauve-700 hover:shadow-sm hover:shadow-purple-900/20">
            ...
            <div className="border border-zinc-700 bg-zinc-900 p-4 space-y-4 rounded-xl transition-all hover:border-mauve-700 hover:shadow-md hover:shadow-purple-900/20">
              ...
              <div className="flex items-center gap-2 w-full">
                <div className="relative flex-1">
                  <button onClick={() => setVersionOpen((v) => !v)} className={`inline-flex w-full items-center justify-between border px-3 py-2 text-sm rounded-md transition-all hover:border-mauve-700 ${versionOpen ? "border-mauve-700 text-mauve-600" : "border-zinc-700 bg-zinc-900 text-zinc-200"}`}>
                    <span>{selectedVersion}</span>
                    <svg viewBox="0 0 24 24" className={`h-4 w-4 transition-colors ${versionOpen ? "text-mauve-600 rotate-180" : "text-zinc-400"}`} aria-hidden>
                      <path d="M6 9l6 6 6-6" fill="none" stroke="currentColor" strokeWidth="2" />
                    </svg>
                  </button>
                  {versionOpen && (
                    <div className="absolute left-0 top-full mt-2 z-50 w-full border border-zinc-700 bg-zinc-900 rounded-md shadow-md shadow-purple-900/20">
                      <div className="py-1">
                        {versionList.map((v) => (
                          <button key={v} onClick={() => { setSelectedVersion(v); setVersionOpen(false); }} className="block w-full text-left px-3 py-2 text-sm text-zinc-200 hover:bg-zinc-800/60 hover:text-mauve-600">
                            {v}
                          </button>
                        ))}
                        <Link href={`/mods/${slug}?tab=files`} className="block px-3 py-2 text-sm text-mauve-600 hover:text-mauve-500">Files</Link>
                      </div>
                    </div>
                  )}
                </div>
                <Link href="#" className="shrink-0 grid place-items-center h-9 w-9 bg-mauve-600 rounded-full transition-all hover:bg-mauve-500 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-purple-900/30">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button className="w-full inline-flex items-center justify-center gap-2 border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-200 rounded-md transition-all hover:border-mauve-700 hover:-translate-y-0.5 hover:shadow-md hover:shadow-purple-900/20">
                  <svg viewBox="0 0 24 24" className="h-5 w-5" aria-hidden>
