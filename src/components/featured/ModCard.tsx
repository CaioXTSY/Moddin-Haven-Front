...
export default function ModCard({ title, author, category, downloads, rating, imageSrc }: Props) {
  ...
  return (
    <Link href={`/mods/${slug}`} className="relative group border border-zinc-800 bg-black transition-all hover:border-purple-700 hover:shadow-xl hover:shadow-purple-900/30 rounded-lg overflow-hidden">
      <div className="relative h-64 w-full md:h-72 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via_black/20 to-black/70" />

        <div className="absolute left-4 top-4 inline-flex items-center gap-2 bg-purple-600 px-2 py-1 text-[11px] font-semibold tracking-wide text-black uppercase rounded-full">
        ...
        <div className="absolute right-4 top-4 flex gap-2">
          <div className="inline-flex items-center gap-1 border border-zinc-700 bg-black/60 px-2 py-1 text-xs text-zinc-200 backdrop-blur transition-colors group-hover:border-purple-700 rounded-full">
            <StatIconDownload />
            {downloads}
          </div>
          <div className="inline-flex items-center gap-1 border border-zinc-700 bg-black/60 px-2 py-1 text-xs text-purple-500 backdrop-blur transition-colors group-hover:border-purple-700 group-hover:text-purple-400 rounded-full">
            <StatIconStar />
            {rating}
          </div>
        </div>
